...
function api(options) {
  ...
  var defaults = {
    port: 8080,
    env: 'development', // development | production // NODE_ENV overwrites this
    accessLog: false,
    errorLog: './error_log',
  ...
  that.options = _.defaults(options, defaults)
  that.controllers = null;

  that.hooks = {};
  that.handles = {};
  that.errorLogStream = null;
  that.cleanupFunc = null;

  // set env
  if(_.has(that.options, 'env')) {
    if(ENV === null) {
      ENV = that.options.env;  
    } else {
      // overwrite options.env with NODE_ENV
      // @todo log warning?!
      that.options.env = ENV;
    }
  }
...
api.prototype.start = function(readyCallback) {
  ...
  if(ENV == 'development') that.connect.use(connect.responseTime());
  ...
  // actually call the controller func
  // handle uncaughtExceptions gracefully
  // var uncaughtExceptionCallback = null;  
  // that.connect.use(function(req, res, next){
  //   uncaughtExceptionCallback = function(err) {
  //     return next(err);
  //   }
  ...
  // that.connect.use(function(req, res, next){
  //   // remove uncaughtExceptionCallback 
  ...
  // error handler
  //that.connect.use(function(err, req, res, next){
  //  // error handler
  //  console.log('error here');
  //  // next(err);
//
  //  that.connect.use(middlewareHooks(that, 'onEnd2', err));
  //  
  //});

  // hook: cleanup
  // that.connect.use(middlewareHooks(that, 'cleanup'));

  // run cleanup function
  // if there was an error, cleanup function will be called in middlewareError
  
  that.connect.use(function(req, res, next){
    if(that.cleanupFunc) that.cleanupFunc.call(null, that);
    next();
  });
  ...
  that.server = http.createServer(that.connect).listen(that.options.port, function() {
    util.log('[api] (' + ENV + ') listening at port ' + that.options.port);
